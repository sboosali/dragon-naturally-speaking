__version__ = "$Revision: 588 $, $Date: 2018-01-26 17:39:27 +0100 (vr, 26 jan 2018) $, $Author: quintijn $"
...
def GrammarElementSort(something, other):
    if IsText(something):
        ssomething=string.lower(something)
    else:
        ssomething=string.lower(something.GetName())
    if IsText(other):
        sother=string.lower(other)
    else:
        sother=string.lower(other.GetName())
...
class GrammarElement:
    ...
    def IsAllText(self):
        return (len(self.Included)>0) and (not self.ObjIncluded)
    ...
    def SetIsLongAlternative(self):
        if self.GramType==AltCode:
            self.IsLA=self.IsAllText() and (len(string.join(self.Included, ' | '))>MaxUnfoldLen)
...
#        AllNames = map(lambda x: x.Name,AllRules) # my first map and lambda; alas! no longer needed :-)
    ...
    def ReduceLongAlternatives(self,contents):
        if len(contents)>MaxUnfoldLen:
            p=string.find(contents[MaxUnfoldLen:],'|')
        ...
        # this if clause gives a significant speedup for long lists                
            ...
            if (not Unfold) and self.IsLongAlternative():
                ...
                if (self.GramType==AltCode) or (self.GramType==ListCode):
                    contents=string.join(self.Included,' | ')
                else:
                    contents=string.join(self.Included)
    ...
    for element in Parser.ruleDefines[name]:
        if element[0]=='start':
            ...
            if CurElement.IsAllText() and (CurElement.GramType==SeqCode):
                    stack[1].Included[-1]=string.join(CurElement.Included)
