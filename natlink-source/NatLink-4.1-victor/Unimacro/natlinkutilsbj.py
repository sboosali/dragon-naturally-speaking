__version__ = "$Revision: 597 $, $Date: 2018-04-21 14:16:45 +0200 (za, 21 apr 2018) $, $Author: quintijn $"
...
##        print 'setting exclusive %s, self: %s'% (exclusive, self)
    ...
    def switchOn(self, **kw):
        ...
        if 'activateRule' in kw:
            ...
            del kw['activateRule']
            if self.mayBeSwitchedOn:
                if type(rule) in (six.binary_type, six.text_type):
                    rule = utilsqh.convertToBinary(rule)
...
class IniGrammar(IniGrammarAncestor):
    ...
    def __init__(self):
        ...
        try:
            ...
            if self.gramSpec:
                if type(self.gramSpec) == six.binary_type:
                    self.gramSpec = utilsqh.convertToUnicode(self.gramSpec)
                if type(self.gramSpec) == six.text_type:
                    self.gramSpec = utilsqh.convertToBinary(self.gramSpec)
                # print 'grammar %s, type: %s'% (self.name, type(self.gramSpec))
                if type(self.gramSpec) == six.binary_type:
...
##            self.version = version.split(',')[0].strip("$Revision: ")
    ...
    def hasCommon(self, one, two, allResults=None, withIndex=None):
      ...
      ((wish:  **as a special case, one can be a word of the original list, if used in a recursive callback! ))
        ...
        L = []
        i = -1
        if type(two) == six.text_type:
            two = [two]
        elif type(two) == six.binary_type:
            two = [utilsqh.convertToUnicode(two)]
        if type(one) == six.text_type:
            one = [one]
        elif type(one) == six.binary_type:
            one = [utilsqh.convertToUnicode(one)]
